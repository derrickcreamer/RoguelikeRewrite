		...
		static void HandleNotifications(object o) {
			switch(o) {
				case FireballEvent.ExplosionNotification n:
					...
					foreach(Point p in n.Event.Target.Value.EnumeratePointsAtManhattanDistance(n.Radius, true)) {
					...
					while(true) {
						if(walkDir != null) {
							if(w.KeyPressed) {
								...
								n.Event.ChosenAction = new WalkEvent(g.Player, g.Player.Position.PointInDir(walkDir.Value));
								Thread.Sleep(30);
								return;
							}
						}
						if(w.KeyPressed) {
							...
							if(dir != null) {
								n.Event.ChosenAction = new WalkEvent(g.Player, g.Player.Position.PointInDir(dir.Value));
